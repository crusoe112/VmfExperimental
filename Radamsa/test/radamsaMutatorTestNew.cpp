...
class RadamasaMutatorTest : public ::testing::Test {
  protected:
  ...
  void testDecrementByte()
  {
    RadamsaDecrementByteMutator theMutator("RadamsaDecrementByteMutator");
    theMutator.init(*config);
    theMutator.registerStorageNeeds(*registry);
    theMutator.registerMetadataNeeds(*metadata);

    // create a test case that will be used in decrement
    ...
    // create a new test case by decrementing the first byte
    StorageEntry* modEntry = storage->createNewEntry();
    try
    {
    ...
    ASSERT_EQ(12, newBufSize) << "Decrement case is not of expected size";
    
    int numDecremented = 0;
    // test that everything after the first index is the same
    for(int b = 0; b < newBufSize; b++)
    {
        if (buff[b] == newBuf[b] + 1) {
            numDecremented++;
        } else if (buff[b] != newBuf[b]) {
            FAIL() << "Values have been changed in unexpected way";
        }
    }
    EXPECT_EQ(1, numDecremented) << "Only one byte should have been decremented";
  }

  TEST_F(RadamsaMutatorTest, TestDecrementByte) 
  { 
      testDecrementByte();
  }
