...
using vmf::RadamsaRepeatLineMutator;
using vmf::BaseException;
using vmf::RuntimeException;

class RadamsaRepeatLineMutatorTest : public ::testing::Test {
  protected:
    RadamsaRepeatLineMutatorTest() 
    {
      ...
      theMutator = new RadamsaRepeatLineMutator("RadamsaRepeatLineMutator");
      config = testHelper -> getConfig();
    }

    ~RadamsaRepeatLineMutatorTest() override = default;
    ...
    RadamsaRepeatLineMutator* theMutator;
...
/*TEST_F(RadamsaRepeatLineMutatorTest, BufferNotNull)
{
    // no way to test this without mocks
}*/

TEST_F(RadamsaRepeatLineMutatorTest, BufferSizeGEOne)
{    
...
TEST_F(RadamsaRepeatLineMutatorTest, OneLine)
{    
    StorageEntry* baseEntry = storage->createNewEntry();
    StorageEntry* modEntry = storage->createNewEntry();
...
TEST_F(RadamsaRepeatLineMutatorTest, TwoLines)
{    
    StorageEntry* baseEntry = storage->createNewEntry();
    StorageEntry* modEntry = storage->createNewEntry();
    ...
    EXPECT_EQ(modBuff_len % line_len, 1);
    // test buff contents
    EXPECT_TRUE((count4 > 1 && count5 == 1) ||
                (count5 > 1 && count4 == 1)
               );
}

TEST_F(RadamsaRepeatLineMutatorTest, ThreeLines)
{    
    StorageEntry* baseEntry = storage->createNewEntry();
    StorageEntry* modEntry = storage->createNewEntry();
    ...
    EXPECT_EQ(modBuff_len % line_len, 1);
    // test buff contents
    EXPECT_TRUE((count4 > 1 && count5 == 1 && count6 == 1) ||
                (count5 > 1 && count4 == 1 && count6 == 1) ||
                (count6 > 1 && count4 == 1 && count5 == 1)
               );
}
